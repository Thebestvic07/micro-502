...
class PID(object):
    ...
    def __call__(self, reference_, vel=None, dt=None):
        ...
        :param vel: If set, uses this value as derrivative of the error instead of difference 
            between error and last error.
        ...
        self._forward = reference_*self.Kf

        # Compute error terms
        error = self.setpoint - reference_
        d_input = reference_ - (self._last_input if (self._last_input is not None) else reference_)
        ...
        if vel is not None:
            self._derivative = -self.Kd * vel
        elif self.differetial_on_measurement:
            self._derivative = -self.Kd * d_input / dt
        else:
            self._derivative = self.Kd * d_error / dt

        # Compute final output
        output = self._forward + self._proportional + self._integral + self._derivative
        output = _clamp(output, self.output_limits)

        # Keep track of state
        self._last_output = output
        self._last_input = reference_
    ...
    def store_reservoir(self):
        self._integral_saved = self._integral
    
    def restore_reservoir(self):
        self._integral = self._integral_saved
